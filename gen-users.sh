...
if [ $ilosc = "" ]; then echo Brak parametru - ilosc uzytkownikow; exit 1; fi
if [ $cpu = "" ]; then echo Brak parametru - nazwa bazy; exit 1; fi
if [ $db = "" ]; then echo Brak parametru - nazwa bazy; exit 1; fi
...
do
	...
	do
		...
		## PORZADKI ##
		#if [ -e ${db}${c}.db ]; then rm -rf ${db}${c}.db; echo "-> plik ${db}${c}.db usuniety !"; fi
		...
		echo -n "$(if [ $i -eq 1 ] && [ $c -eq 1 ]; then echo -n "["; else echo -n ","; fi ) 
		    {
			\"age\" : ${age},
			\"dances\" : [
				{
	...
	lu=$(( $(date +%s) - $start ))
	echo "Generowanie $ilosc uzytkownikow (k=$ck m=$cm) zajelo $lu sekund"
...
echo
echo "
wielkosc bazy: $(ls -l -h $db*.db | cut -d" " -f5)

-----------------------------------------------------
Calkowity czas dzialania programu $la sekund

$(date)

"
echo "$(date) -- usr=$ilosc -- lu=$lu sek    la=$la sek" >> gen.log
